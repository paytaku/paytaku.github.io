...
{
    name: "PayPayカード → Revolut → ANA Pay → 楽天Edy → 楽天Pay（チャージ払い）",
    pays: [
      "楽天Pay",
      "楽天Edy",
      "ANA Pay"
    ],
    total: "合計 2.5%（各種ボーナス込みで最大3.5%＋PayPayStep+0.5%）",
    steps: [
    ...
    gains: [
      "PayPayカード→Revolutチャージ：1.0%（最大1.5%、PayPayStep達成で+0.5%）",
      "Revolut→ANA Payチャージ：0%（月10万円まで、経由地としての利用）",
      "ANA Pay→楽天Edyチャージ：0.5%",
      "楽天Edy→楽天Pay（チャージ払い）チャージ：0%（月1万円まで）",
      "楽天Payでのお店決済：1.0%（最大1.5%、楽天ポイントカード提示2回以上で+0.5%）"
    ],
    split: [
      { pt: "PayPayポイント", rate: 1.0, note: "PayPayカード→Revolutチャージ分（PayPayStep達成で+0.5%）" },
      { pt: "-", rate: 0, note: "Revolut→ANA Payチャージ分（月10万円まで、還元なし）" },
      { pt: "ANAマイル", rate: 0.5, note: "ANA Pay→楽天Edyチャージ分" },
      { pt: "-", rate: 0, note: "楽天Edy→楽天Pay（チャージ払い）チャージ分（月1万円まで、還元なし）" },
      { pt: "楽天ポイント", rate: 1.0, note: "楽天Payでのお店決済分（ポイントカード提示2回以上で+0.5%）" }
    ],
    starter: "PayPayカード（Mastercard）。⚠️【2026年8月26日追記】2026年6月2日からPayPayカードは「他社決済へのチャージ」がポイント付与対象外になったことが確定情報として確認できました。Revolutは他社決済サービスに該当するため、このルートの起点（PayPayカード→Revolut：1.0%）は現在機能していない可能性が高いです。掲載は残しますが、実施前に必ず最新のPayPayカード規約を確認してください。",
    howto: {
      prep: [
        "PayPayカードを発行する（Mastercardブランドを選択）",
        "Revolut・ANA Pay・楽天Edy・楽天ペイの各アプリをインストールする"
      ],
      flow: [
        "RevolutにPayPayカードからチャージする",
        "Revolut（Visaデビット）でANA Payにチャージする（月10万円まで）",
        "ANA Payで楽天Edyにチャージする",
        "楽天Edyで楽天Pay（チャージ払い）にチャージする（月1万円まで）",
        "お店で楽天Payとして支払う（楽天ポイントカードも提示すると上乗せ）"
      ],
      time: "カード発行に1〜2週間。以降のチャージ操作は月15分ほど"
    },
    note: "経由地が多く、各段階に月間上限があるため（ANA Pay10万円・楽天Pay（チャージ払い）1万円など）、最終的に回せる金額は一番低い上限に引っ張られます。⚠️2026年6月2日からPayPayカード→他社決済サービスへのチャージはポイント付与対象外と確定しています（Z家のお得メモ「チャージルート関係の記録」で確認）。このルートのPayPayカード→Revolut部分（1.0%）は現在成立しない可能性が高いため、要最新確認です。［各サービスの最新のチャージ上限・対応状況は要最新確認・確認日: 2026-08-26］",
    url: "https://poinmy.com/payment/?mode=route",
    caution: "⚠️2026年6月2日からPayPayカードの他社決済へのチャージはポイント付与対象外。Revolutも対象に含まれる可能性が高く、起点の1.0%は現在成立しない見込み。ANA Payへのチャージは月10万円、楽天Pay（チャージ払い）へのチャージは月1万円が上限。楽天ポイントカードの提示ボーナスは、カウント期間中2回以上の提示が条件。",
    atStore: {
      rate: "2.5%",
      method: "楽天Payのチャージ払いで支払い、楽天ポイントカードも合わせて提示"
    },
    starters: {
      "PayPayカード": 1.0
    }
...
// ========== チャージルート：共通区間（セグメント）＋起点カード（コネクタ） ==========
// 複数の起点カードが同じ経路に合流するケースが増えてきたため、
// 「入口ノードから先が固定の区間」をセグメントとして1回だけ定義し、
// 「どのカードがいくらの還元でどのセグメントの入口まで届くか」をコネクタとして
// 積み上げる方式にしている。新しい起点カードが見つかったら ROUTE_CONNECTORS に
// 1件足すだけで、対応するセグメントと自動的に組み合わさったルートが増える。
// steps/gains/split/atStore/caution はここが唯一の情報源（Single Source of Truth）。
// 内容を直すときはセグメント側だけ直せば、そのセグメントを使う全ルートに反映される。
...
function renderStores(){
  ...
  // 「すべて」×未検索×還元率順でもない、いわゆる初期表示（登録店舗を全件表示）は
  // 件数が多く、カード内の詳細（card-options）を全部展開すると重くなる原因になるため、
  // この場合だけ最初は折りたたんで表示し、タップした店舗だけ開くようにする。
  // カテゴリで絞り込んだ時・検索した時・還元率順にした時は、これまで通り展開表示のまま。
  const heavyDefaultView = !q && storeState.category === "all" && !storeState.sortByRate;
  ...
  filtered.forEach(store => {
    ...
    const card = buildStoreCardEl(store);
    if(heavyDefaultView){
      card.classList.add("collapsed"); // 初期の全件表示は軽量化のため折りたたみ
    } else {
      card.classList.remove("collapsed");
    }
    list.appendChild(card);
...
function renderRouteChips(){
  ...
  // 起点カードと同じく、リスト／ボタンを切り替えられるようにする
  const toggleWrap = document.createElement("div");
  toggleWrap.style.cssText = "width:100%;margin-bottom:6px;";
  toggleWrap.innerHTML = pickerStyleToggleHtml("goal");
  row.appendChild(toggleWrap);
  bindPickerStyleToggle(toggleWrap, "goal", renderRoutes);

  const style = getPickerStyle("goal");
  if(style === "list"){
    const sel = document.createElement("select");
    sel.className = "answer-select";
    sel.id = "goalSel";
    sel.innerHTML = `<option value="all" ${routeFilter==="all"?"selected":""}>すべて</option>` +
      keys.map(p => `<option value="${p}" ${routeFilter===p?"selected":""}>${p}</option>`).join("");
    sel.addEventListener("change", ()=>{ routeFilter = sel.value; renderRoutes(); });
    row.appendChild(sel);
    return;
  }
...
const STARTER_CARDS = [
  ...
  { name: "JQ CARDセゾンGOLD", rate: 0.5, pt: "永久不滅ポイント",
    note: "ファミペイへのチャージで0.5%（月2万円まで）。年間100万円利用で10,000pt、年間50万円利用で2,500ptの別枠ボーナスあり。" },
  { name: "住信SBIデビットカード Point+", rate: 1.25, pt: "Vポイント",
    note: "モバイルnanacoへのチャージで1.25%（iPhone限定）。" },
];
let starterIdx = 0;

// ===== 起点カード／ゴール決済方法：表示スタイル（リスト or ボタン）の切り替え =====
// 「起点はリストなのにゴールはボタンなのはなぜ？」という声を受けて、
// どちらも好きな見た目に切り替えられるようにする。選択はこの端末に保存される。
const PICKER_STYLE_KEY_STARTER = "pt_picker_style_starter";
const PICKER_STYLE_KEY_GOAL    = "pt_picker_style_goal";
function getPickerStyle(which){
  const key = which === "goal" ? PICKER_STYLE_KEY_GOAL : PICKER_STYLE_KEY_STARTER;
  const fallback = which === "goal" ? "button" : "list"; // 従来の見た目をデフォルトのまま維持
  try{
    const v = localStorage.getItem(key);
    return (v === "list" || v === "button") ? v : fallback;
  } catch { return fallback; }
}
function setPickerStyle(which, style){
  const key = which === "goal" ? PICKER_STYLE_KEY_GOAL : PICKER_STYLE_KEY_STARTER;
  try{ localStorage.setItem(key, style); } catch {}
}
function pickerStyleToggleHtml(which){
  const cur = getPickerStyle(which);
  return `<div class="picker-style-toggle" data-picker-style-toggle="${which}" style="display:inline-flex;gap:4px;margin-left:8px;vertical-align:middle;">
    <button type="button" class="picker-style-btn${cur==='list'?' active':''}" data-style="list" style="font-size:11px;padding:3px 8px;border-radius:6px;border:1px solid var(--border,#ccc);background:${cur==='list'?'var(--primary)':'transparent'};color:${cur==='list'?'#fff':'inherit'};cursor:pointer;">リスト</button>
    <button type="button" class="picker-style-btn${cur==='button'?' active':''}" data-style="button" style="font-size:11px;padding:3px 8px;border-radius:6px;border:1px solid var(--border,#ccc);background:${cur==='button'?'var(--primary)':'transparent'};color:${cur==='button'?'#fff':'inherit'};cursor:pointer;">ボタン</button>
  </div>`;
}
function bindPickerStyleToggle(container, which, onChange){
  const el = container.querySelector(`[data-picker-style-toggle="${which}"]`);
  if(!el) return;
  el.querySelectorAll(".picker-style-btn").forEach(b=>{
    b.addEventListener("click", ()=>{
      setPickerStyle(which, b.dataset.style);
      onChange();
    });
  });
}
...
function routeWithStarter(route){
  // 「ゴール決済方法で探す」タブを見ている間は、起点カードの選択を反映しない。
  // （起点タブで選んだカードがゴール検索の絞り込み・並び替え・表示に
  //   漏れて影響してしまっていたのを防ぐため）
  if(activeRouteTab === "goal") return { usable: true, base: null };
...
function starterPickerHtml(){
  const st = STARTER_CARDS[starterIdx];
  const style = getPickerStyle("starter");
  const picker = style === "button"
    ? `<div class="category-chip-row" id="starterChipRow">
        ${STARTER_CARDS.map((c,i)=>`
          <button type="button" class="category-chip${i===starterIdx?" active":""}" data-starter-idx="${i}" style="white-space:normal;text-align:center;min-width:64px;">${c.name}</button>
        `).join("")}
      </div>`
  ...
  return `<div class="starter-picker">
    <span class="mypanel-label">チャージの起点にするカード${pickerStyleToggleHtml("starter")}</span>
    ${picker}
...
// 「起点カードで探す」と「ゴール決済方法で探す」は見る観点が違うため、
// 今どちらのタブを見ているか（activeRouteTab）を覚えておき、
// 表示中のタブの条件だけでルート一覧を絞り込む。
// （以前は起点カードの選択がゴール検索側にも影響してしまっていたため分離）
let activeRouteTab = "starter";
...
function renderRoutes(){
  ...
  if(sb){
    ...
    bindPickerStyleToggle(sb, "starter", renderRoutes);
  }
  const list = document.getElementById("routeList");
  list.innerHTML = "";
  // ゴール（使用する決済方法）での絞り込みは「ゴール決済方法で探す」タブの時だけ効かせる。
  // 起点タブを見ている間はゴールの選択を無視して全ルートを対象にする。
  let shown = CHARGE_ROUTES.filter(r =>
    activeRouteTab !== "goal" || routeFilter === "all" || (r.pays || []).includes(routeFilter));
  // 起点カードを選んでいるときは、そのカードで実行できるルートだけを、
  // そのカードでの還元率が高い順に並べる（起点タブを見ている時のみ）。
  const st0 = activeRouteTab === "starter" ? STARTER_CARDS[starterIdx] : null;
